...
const SURGE_WARNING_LEGEND = `
  ...
  <p class="muted" style="font-size:11px;margin:0 0 2px;opacity:0.65;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
    <span>📈 10년 상승 — 최근 10년 전체 상승률 ÷ 상장연수(최대 10년)<br>🛡️ 10년 승률 — 최근 10년 매월 상승 마감한 비율(❗=상장 10년 미만)</span>
  </p>
`;
...
const RANKING_ENTRIES = [
  ...
  // 2026-09-04 개편: 상승 압력 → 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년)), 투자 안정 → 10년 승률(구 우상향점수와 통합, 명칭 통일)
  ...
  return `
    ...
    <div class="wizard-root-options">
      <button type="button" class="wizard-root-option" data-wizard-action="branchC-style-short">
        <b>A. 단기적인 수익을 원함(▲600%~▼60%)</b><br><span class="wizard-option-sub">(10년 연평균 상승률(전체 상승률÷상장연수, 최대 10년) 높은 주식) — S&amp;P 500중 10년 상승 높은순위 30위까지</span>
      </button>
...
// 자동찾기(2026-09-04 개편): 투자처별로 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년))+10년 승률 합계 상위 30 —
...
function combinedRankTableHtml(rows, universeLabel, rowNameHtmlFn, priceStrFn) {
  const body = rows
    ...
    .join("");
  return `
    <p class="disclaimer tab-note"><span style="filter:grayscale(1);">📢</span> ${universeLabel} 중 거래대금(최근 5일 평균)이 큰 순 30개입니다. 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년))·10년 승률은 매일 자동 갱신되는 배치 DB 기준이며 투자 자문이 아닙니다.</p>
    <table class="top30-table">
...
const ASSET_TREND_METRICS = {
  ...
  // 2026-09-04 개편: 상승 압력 → 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년)), 투자 안정 → 삭제(10년 승률로 대체, winrate 항목과 통합)
  pressure: {
    icon: "rocket",
    label: "10년 상승",
    ...
    note: "10년 상승(최근 10년 전체 상승률 ÷ 10, 상장 10년 미만은 상장일부터 지금까지의 전체 상승률 ÷ 상장연수)이 높은 순 순위입니다.",
    noRiskCol: true,
...
function historicalTableHtml(rows, rankColumnLabel, periodLabel = "1년전") {
  ...
  return `
    <p class="disclaimer tab-note"><span style="filter:grayscale(1);">📢</span> 10년 상승(최근 10년 연평균 상승률(전체 상승률÷상장연수, 최대 10년))·10년 승률(10년 월간 상승 마감 비율)은 매일 갱신되는 배치 DB의 현재값 기준 참고용 지표입니다. 투자 자문이 아닙니다.</p>
    <table class="top30-table">
      <thead>
  ...
  async function paintUpTo(targetCount) {
    try {
      ...
      historicalResults.innerHTML = `
        <p class="disclaimer tab-note"><span style="filter:grayscale(1);">📢</span> ${periodLabel}(코스피200+코스닥150 대상) 대비 ${rankLabel} 기준이며, 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년))·10년 승률은 매일 갱신되는 배치 DB 기준입니다. 투자 자문이 아닙니다.</p>
...
async function runHistoricalMoversAsset(section, period, direction) {
  ...
  try {
    ...
    historicalStatus.style.display = "none";
    historicalResults.innerHTML = `
      <p class="disclaimer tab-note"><span style="filter:grayscale(1);">📢</span> ${universeLabel} 대상 ${periodLabel} 대비 ${rankLabel} 순위이며, 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년))·10년 승률은 매일 갱신되는 배치 DB 기준입니다. 투자 자문이 아닙니다.</p>
      <table class="top30-table">
...
function moversTableHtml(scored, rankNote) {
  ...
  return `
      <p class="disclaimer tab-note"><span style="filter:grayscale(1);">📢</span> ${rankNote} 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년))·10년 승률은 매일 갱신되는 배치 DB 기준이며 투자 자문이 아닙니다.</p>
...
async function runTrendRsiWinRate(mode) {
  ...
  const isRet = mode === "ret"; // 10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년)) 순위 — 2026-09-04 상승압력 대체
      ...
      resultsEl.innerHTML = `
        ${
          isRsi
            ...
            : isRet
            ? `<p class="disclaimer tab-note"><span style="filter:grayscale(1);">📢</span> ${universeLabel} 대상 — 10년 상승(최근 10년 전체 상승률 ÷ 10, 상장 10년 미만은 상장일부터 지금까지의 전체 상승률 ÷ 상장연수)이 높은 순 순위입니다. 참고용 지표이며 투자 자문이 아닙니다.</p>`
...
async function renderFutureModalHeader(ticker, quote, metricsPromise, marketReturnsPromise) {
  ...
  try {
    ...
    if (scoresEl) {
      scoresEl.innerHTML = `
        <span class="mini-score-circle small" title="10년 상승(연평균 상승률(전체 상승률÷상장연수, 최대 10년))">${ret10}</span>
