...
function ensureExtraDataLoaded(market) {
  ...
  const src = market === "domestic" ? "data/kr-data-extra.js?v=20260904h" : "data/sp500-data-extra.js?v=20260904h";
...
const CRYPTO_PURPOSE = (() => {
  ...
  // 2026-09-14 사용자 지적(밈코어 M이 스테이블코인으로 분류됨) 후 200개 전수 점검:
  // M→밈, KHYPE(스테이킹 HYPE)→스테이킹·랩드, BTW(Bitway, BTC 금융 플랫폼)→금융서비스로 바로잡고
  // "기타"에 몰려 있던 코인 중 용도가 분명한 것(EURC·U 스테이블, PENGU·SPX 밈, 디파이·L1/L2·AI 인프라 등)을 제자리로 옮김
  put("가치저장", "BTC WBTC CBBTC BTCB LBTC BTCT XAUT PAXG GRAM");
  put("결제·송금", "XRP XLM LTC BCH DASH BSV XEC");
  put(
    "스테이블코인",
    "USDT USDC USDS DAI USDE USD USD1 USDT0 USDG PYUSD RLUSD USDY USDD BFUSD SUSDE USDGO USDF USDCE AETHUSDT SYRUPUSDC TUSD FDUSD EURC U"
  );
  put(
    "플랫폼·스마트컨트랙트",
    "ETH SOL ADA TRX AVAX DOT NEAR SUI TON ICP ETC HBAR ALGO ATOM MNT POL KAS XTZ EGLD SEI APT ARB OP PI " +
      "A BERA CC CFX ELF GAS IOTA LINEA MEGA MINA MON NEO QTUM SOON STX TIA VET XPL ZETA ZIL ZK PLUME PROS"
  );
  put(
    "금융서비스",
    "BNB LEO CRO OKB BGB GT KCS HTX UNI AAVE SKY MKR ONDO ENA MORPHO JLP JST HYPE ASTER WLFI LDO INJ COMP CRV SNX CAKE RAY JUP " +
      "BTW 1INCH AERO BABY CFG CHIP COW DEEP DRV EDGE ETHFI FF FLUID JTO KMNO LIT ORCA PENDLE RAIN SPK SUN SYRUP XCN ZRX"
  );
  put("스테이킹·랩드", "STETH WSTETH WBETH WEETH RSETH RETH LSETH JITOSOL BNSOL WETH AETHWETH WBNB WTRX KHYPE");
  put("AI·데이터", "LINK TAO VVV WLD FET GRT RENDER RNDR FIL AR THETA TFUEL AKT ATH AWE BTT DATA GEOD GLM IO KAITO KITE LPT OPEN PYTH SENT TRAC VIRTUAL WAL");
  put("프라이버시", "XMR ZEC ZAMA");
  put("밈·커뮤니티", "DOGE SHIB PEPE PUMP BONK WIF FLOKI TRUMP FARTCOIN BRETT M PENGU SPX");
  put("게임·메타버스", "SAND MANA AXS IMX GALA ENJ RON BEAM SUPER NXPC");
  put("NFT", "APE BLUR");
  return m;
})();
...
function renderCompanyBubble(leaf, sectorColorValue) {
  ...
  // 균등 모드: 예전처럼 은은한 작은 글씨(사용자 확정) / 시총 모드: 잘리지 않는 선에서 타일 폭을 꽉 채움
  // (한글은 글자폭 1em, 영문/숫자는 약 0.62em으로 계산 — finviz 방식)
  if (sizeMode === "equal") {
    nameEl.style.fontSize = `${Math.max(8, Math.min(22, minDim * (isKrView ? 0.16 : 0.22)))}px`;
  } else {
    let nameUnits = 0;
...
// 지도 중심 미세 조정 이력: 60(좌측) → 0 → 20(10px 왼쪽) → -20(20px 오른쪽) → 0(2026-08-30 "10px 왼쪽" 재조정 = 정중앙, 국내/해외·전체보기/접기 공통)
// view.x에서 이 값의 절반만큼 왼쪽으로 이동함(음수면 오른쪽)
