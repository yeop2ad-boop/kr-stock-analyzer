...
# [추가 지표(2026-09-04)] wr1y=직전12개월 승률%, ret10y=연복리 수익률%(CAGR — 매년 몇 %씩 오른 셈, 10년 미만은 상장 후 기간 연율화), ret1y=직전12개월 상승률%,
    ...
    for ($attempt = 1; $attempt -le 3; $attempt++) {
      try {
        ...
        if ($sorted[0].c -gt 0 -and $total -ge 12) {
          # 2026-09-04 사용자 최종 확정(4차, "복리기준으로"): 연복리 수익률 CAGR — 매년 몇 %씩 오른 셈(10년 미만은 상장 후 기간 연율화)
